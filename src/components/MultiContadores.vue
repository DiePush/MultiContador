...
export default {
  data() {
    return {
      ...
      tiempoGlobalRestante: JSON.parse(localStorage.getItem('tiempoGlobalRestante')) || 900, // Tiempo global inicial (15 minutos)
      ultimaActualizacion: JSON.parse(localStorage.getItem('ultimaActualizacion')) || Date.now()
    };
  },
  methods: {
    confirmarNombre() {
    ...
    guardarDatos() {
      ...
      localStorage.setItem('historial', JSON.stringify(this.historial));
      localStorage.setItem('tiempoGlobalRestante', JSON.stringify(this.tiempoGlobalRestante));
      localStorage.setItem('ultimaActualizacion', JSON.stringify(Date.now()));
    },
    calcularTiempoRestante() {
      const ahora = Date.now();
      const tiempoTranscurrido = (ahora - this.ultimaActualizacion) / 1000;
      this.tiempoGlobalRestante = Math.max(this.tiempoGlobalRestante - tiempoTranscurrido, 0);
      this.guardarDatos();
    }
  },
  mounted() {
    this.calcularTiempoRestante();
    if (this.cuentaRegresivaActiva) {
      this.iniciarCuentaRegresiva();
    }
  }
};
